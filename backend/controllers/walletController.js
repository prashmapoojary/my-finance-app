...
exports.createWallet = async (req, res) => {
    ...
    try {
        await db.query('INSERT INTO wallets (name, balance, userid) VALUES ($1, $2, $3)', 
...
exports.getWallets = async (req, res) => {
    ...
    try {
        const { rows: wallets } = await db.query('SELECT * FROM wallets WHERE userid = $1', [userId]);
        res.json(wallets);
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'Server error' });
...
exports.deleteWallet = async (req, res) => {
    ...
    try {
        ...
        const result = await db.query('DELETE FROM wallets WHERE id = $1 AND userid = $2', [walletId, userId]);
