...
exports.addTransaction = async (req, res) => {
    ...
    try {
        ...
        const { rows: walletCheck } = await db.query('SELECT * FROM wallets WHERE id = $1 AND userid = $2', [walletId, userId]);
        ...
        await db.query(
            'INSERT INTO transactions (walletid, type, amount, category, description, date) VALUES ($1, $2, $3, $4, $5, $6)',
...
exports.deleteTransaction = async (req, res) => {
    ...
    try {
        ...
        const { rows: trx } = await db.query(
            `SELECT t.* FROM transactions t 
             JOIN wallets w ON t.walletid = w.id 
             WHERE t.id = $1 AND w.userid = $2`, 
